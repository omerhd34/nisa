...
// Site verilerini getirme - fallback mekanizması ile
async function getSiteDataFromDB(section) {
  try {
    ...
    // Veritabanında veri yoksa data.js'den al
    return data[section] || null;
  } catch (error) {
    console.error(`Error fetching ${section} from database:`, error);
    // Hata durumunda data.js'den döndür
    return data[section] || null;
  }
}
...
// Veritabanını seed etme (ilk kurulum için)
